...
class BlackLittermanModel:
    def __init__(self, risk_free_rate: float = 0.04, frequency: int = 252) -> None:
        """
    ...
    def calculate_risk_aversion(
        self, benchmark_return: float, benchmark_var: float
    ) -> float:
        """
    ...
    def calculate_implied_returns(
        self, covariance: pd.DataFrame, market_weights: pd.Series, risk_aversion: float
    ) -> pd.Series:
        """
    ...
    def get_posterior_returns(self, pi, Sigma, P, Q, Omega, tau):
        ...
        combined_return_vector = (
            market_precision @ pi.values.reshape(-1, 1) + P.T @ np.linalg.inv(Omega) @ Q
        )
